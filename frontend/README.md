...
`npm install` and then `npm start` from the project root directory. Then
...
To run the unit tests all you need is `npm test`. For end-to-end test you need to run `npm run cy:run` to run on console or `npm run cy:open` to use the Cypress IDE.

...
const ServiceStatus = () => {
  ...
  return (
    <>
      <Header aria-label={componentTexts.completeTitle} className="spar-header">
        <SkipToContent />
      ...
      <FlexGrid className="status-page-grid">
        <Row>
          <Column>
            <h3 className="title">SPAR Dependency Status</h3>
          </Column>
        </Row>
                ...
                subtitle={`${
                  statusQueries.filter((query) => query.status === "error")
                    .length
                       ...
                       ${SPAR_DEPENDENCIES.length} service${
                  SPAR_DEPENDENCIES.length > 0 ? "s" : ""
                }
                       are down.
                      `}
                /*

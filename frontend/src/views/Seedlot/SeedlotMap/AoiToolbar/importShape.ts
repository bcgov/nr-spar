 ...
 */
type ImportKind = 'kml' | 'kmz' | 'shp-zip' | 'shp' | 'geojson' | 'unknown';

const detectKind = (file: File): ImportKind => {
...
export const importShapeFile = async (file: File): Promise<ImportResult> => {
  ...
  switch (kind) {
    ...
    default:
      throw new Error(
        `Unsupported file type: ${file.name}. Accepted: .kml, .kmz, .geojson, .json, .zip (shapefile), .shp`
      );
  }

...
--    One row per seedlot (enforced by a unique constraint on seedlot_number);
--    holds the natural-stand collection polygon.
--    PostGIS extension already enabled (V31).
--    Geometry stored in WGS-84 (SRID 4326) to match the Leaflet map GeoJSON;
--    the backend has no reprojection library, so storing lon/lat avoids a
--    server-side transform on save.
--    feature_area / feature_perimeter are computed geodesically via
--    ST_Area / ST_Perimeter on the geography cast (true m² / m).
-- -----------------------------------------------------------------------------
create table spar.seedlot_collection_geometry (
  ...
  constraint seedlot_collection_geometry_pk
    primary key (id),
  constraint seedlot_collection_geometry_seedlot_uk
    unique (seedlot_number),
...
comment on column spar.seedlot_collection_geometry.id                is 'Surrogate primary key (UUID, generated by the application).';
comment on column spar.seedlot_collection_geometry.seedlot_number    is 'The seedlot number. Unique FK to spar.seedlot (one row per seedlot).';
comment on column spar.seedlot_collection_geometry.geometry          is 'The collection area polygon or multipolygon in WGS-84 (SRID 4326), matching the map frontend GeoJSON.';
...
comment on column spar.seedlot_collection_geometry.feature_area      is 'The geodesic area of the collection polygon (m²), from ST_Area on the geography cast.';
comment on column spar.seedlot_collection_geometry.feature_perimeter is 'The geodesic perimeter of the collection polygon (m), from ST_Perimeter on the geography cast.';

...
const EditAClassApplicationForm = ({
  isReview,
  applicantData,
  setApplicantData,
}: props) => {
  ...
  const convertToSeedlotForm = (
    ...
    setApplicantData({
      client: getForestClientStringInput(
        "edit-client-read-only",
        applicantClientNumber!
      ),
      locationCode: getStringInputObj(
        "edit-seedlot-location-code",
        seedlot.applicantLocationCode
      ),
      email: getStringInputObj(
        "edit-seedlot-email",
        seedlot.applicantEmailAddress
      ),
      species: getOptionsInputObj(
        "edit-seedlot-species",
        getSpeciesOptionByCode(seedlot.vegetationCode, vegCodes)
      ),
      sourceCode: getStringInputObj(
        "edit-seedlot-source-code",
        seedlot.seedlotSource.seedlotSourceCode
      ),
      willBeRegistered: getBooleanInputObj(
        "edit-seedlot-will-be-registered",
        seedlot.intendedForCrownLand
      ),
      isBcSource: getBooleanInputObj(
        "edit-seedlot-is-bc-source",
        seedlot.sourceInBc
      ),
    });
  };
  ...
  const seedlotPatchMutation = useMutation({
    mutationFn: (payload: SeedlotPatchPayloadType) =>
  ...
  return (
    <FlexGrid
      className={`edit-a-class-seedlot-page ${isReview ? "no-padding" : null}`}
    >
      {isReview ? null : (
        <>
          ...
          <Row className="title-row">
            <PageTitle title="Applicant and seedlot information" />
          </Row>
        </>
      )}
        ...
        <Row className="error-row">
          <Column>
            <ActionableNotification
              ...
              onClose={() => false}
            >
              An unexpected error occurred while saving your edits. Please try
              again, and if the issue persists, contact support.{" "}
              {`${(seedlotPatchMutation.error as AxiosError).code}: ${
                (seedlotPatchMutation.error as AxiosError).message
              }`}
            </ActionableNotification>
          </Column>
        </Row>

...
// Breadcrumbs uses useNavigate from react-router (ESM bundle) which conflicts with
// the CJS bundle used by BrowserRouter in jsdom. Stub it out — it's not under test.
vi.mock('../../../components/Breadcrumbs', () => ({
  default: () => <nav data-testid="breadcrumbs-mock" />
}));

// PurityContent queries impurity codes via getCodesByActivity
vi.mock('../../../api-service/consep/testCodesAPI', () => ({
  getCodesByActivity: () => Promise.resolve([])
}));
...
describe('PurityContent optimistic-lock conflict', () => {
  ...
  it('shows the conflict banner when an update returns 409', async () => {
    ...
    // Wait for data to load - use the actual title "Purity for lot 64132"

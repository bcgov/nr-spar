...
import { getBClassSeedlotProgressStatus } from '../../../../api-service/seedlotAPI';
...
const BClassFormProgress = (
  ...
  const [
    progressStatus,
    setProgressStatus
  ] = useState<BClassProgressIndicatorConfig>(initialProgressConfig);
  ...
  const formProgressStatusQuery = useQuery({
    queryKey: ['seedlots', 'b-class-form-progress', 'status', seedlotNumber],
    queryFn: () => getBClassSeedlotProgressStatus(seedlotNumber ?? ''),
  ...
  useEffect(() => {
    if (formProgressStatusQuery.status === 'success' && (seedlotStatusCode === 'PND' || seedlotStatusCode === 'INC')) {
      const retrievedProgress = formProgressStatusQuery.data
        .data as unknown as BClassProgressIndicatorConfig;
    ...
    return (
      <SeedlotRegistrationProgressClassB
        ...
        interactFunction={(e: number) => {
          navigate(`${addParamToPath(ROUTES.SEEDLOT_B_CLASS_REGISTRATION, seedlotNumber ?? '')}?step=${e + 1}`);
        }}
      />
    );
  };
      ...
      <Row>
        <Column>
          <Button
            ...
            renderIcon={Edit}
            onClick={() => navigate(addParamToPath(ROUTES.SEEDLOT_B_CLASS_REGISTRATION, seedlotNumber ?? ''))}
...
export default BClassFormProgress;

...
import {
  getAClassSeedlotDraft,
  getAClassSeedlotFullForm,
  getSeedlotById,
  putAClassSeedlot,
...
import {
  INITIAL_GEN_WORTH_VALS,
  INITIAL_GEO_INFO_VALS,
} from "../SeedlotReview/constants";
import {
  MeanGeomInfoSectionConfigType,
  RowItem,
...
const ContextContainerClassA = ({ children }: props) => {
  ...
  const [progressStatus, setProgressStatus] = useState<ProgressIndicatorConfig>(
    () => initProgressBar(formStep, initialProgressConfig)
  );

  const [allStepCompleted, setAllStepCompleted] = useState<boolean>(false);
  const [lastSaveTimestamp, setLastSaveTimestamp] = useState<string>(() =>
    DateTime.now().toISO()
  );
  ...
  const [genWorthInfoItems, setGenWorthInfoItems] = useState<
    Record<keyof RowItem, InfoDisplayObj[]>
  >({} as Record<keyof RowItem, InfoDisplayObj[]>);
  const [weightedGwInfoItems, setWeightedGwInfoItems] = useState<
    Record<keyof RowItem, InfoDisplayObj>
  >({} as Record<keyof RowItem, InfoDisplayObj>);
  ...
  const [summaryConfig, setSummaryConfig] = useState<
    typeof SummarySectionConfig
  >(() => structuredClone(SummarySectionConfig));
  const [meanGeomInfos, setMeanGeomInfos] =
    useState<MeanGeomInfoSectionConfigType>(() =>
      structuredClone(defaultMeanGeomConfig)
    );
  const [areaOfUseData, setAreaOfUseData] = useState<AreaOfUseDataType>(
    () => initialAreaOfUseData
  );
  ...
  const [geoInfoVals, setGeoInfoVals] = useState<GeoInfoValType>(
    () => INITIAL_GEO_INFO_VALS
  );
  ...
  const getDefaultLocationCode = (): string =>
  ...
  const updateStepStatus = (
    ...
    if (stepName === "collection") {
      ...
      if (!stepStatus.isInvalid) {
        stepStatus.isComplete = verifyCollectionStepCompleteness(
    ...
    if (stepName === "ownership") {
      ...
      if (!stepStatus.isInvalid) {
        stepStatus.isComplete = verifyOwnershipStepCompleteness(
          allStepData.ownershipStep
        );
      }
    }
    if (stepName === "interim") {
      ...
      if (!stepStatus.isInvalid) {
        stepStatus.isComplete = verifyInterimStepCompleteness(
          allStepData.interimStep
        );
      }
    }
    if (stepName === "orchard") {
      ...
      if (!stepStatus.isInvalid) {
        stepStatus.isComplete = verifyOrchardStepCompleteness(
    ...
    if (stepName === "parent") {
      ...
      if (!stepStatus.isInvalid) {
        stepStatus.isComplete = verifyParentStepCompleteness(
          allStepData.parentTreeStep
        );
      }
    }
    if (stepName === "extraction") {
      stepStatus.isInvalid = validateExtractionStep(
        allStepData.extractionStorageStep
      );
      if (!stepStatus.isInvalid) {
        stepStatus.isComplete = verifyExtractionStepCompleteness(
          allStepData.extractionStorageStep
        );
      }
    }
  ...
  const gameticMethodologyQuery = useQuery({
    ...
    select: (data) =>
      getMultiOptList(data, true, false, true, [
        "isFemaleMethodology",
        "isPliSpecies",
      ]),
    staleTime: THREE_HOURS,
  ...
  useEffect(() => {
    if (seedlotQuery.status === "success") {
      const seedlotStatusCode =
        seedlotQuery.data?.seedlot.seedlotStatus.seedlotStatusCode;
  ...
  }, [getAllSeedlotInfoQuery.status, allStepData]);
  ...
  const updateProgressStatus = (
    ...
    if (currentStepName !== "extraction" && prevStepName === "extraction") {
      clonedStatus.extraction = updateStepStatus(
        "extraction",
        clonedStatus.extraction
      );
    }

    // Set the current step's current val to true, and everything else false
    const stepNames = Object.keys(clonedStatus) as Array<
      keyof ProgressIndicatorConfig
    >;
  ...
  const submitSeedlot = useMutation({
    mutationFn: (payload: SeedlotAClassSubmitType) =>
      putAClassSeedlot(seedlotNumber ?? "", payload),
    onSuccess: () => {
      navigate({
  ...
  const updateAllStepStatus = (): ProgressIndicatorConfig => {
    ...
    const stepNames = Object.keys(
      clonedStatus
    ) as (keyof ProgressIndicatorConfig)[];
  ...
  const saveProgress = useMutation({
    mutationFn: () => {
      ...
      return putAClassSeedlotProgress(seedlotNumber ?? "", {
        allStepData,
    ...
    onSettled: (res, error) => {
      ...
      if (res || (isAxiosError(error) && error.response?.data.status !== 409)) {
        setTimeout(() => {
          setSaveStatus(null);
          setSaveDescription(smartSaveText.idle);
        }, FIVE_SECONDS);
  ...
  useEffect(() => {
    if (
      numOfEdit.current >= MAX_EDIT_BEFORE_SAVE &&
      isFormIncomplete &&
      saveStatus !== "conflict"
    ) {
      if (!saveProgress.isLoading) {
  ...
  }, [getFormDraftQuery.status, getFormDraftQuery.fetchStatus]);

  const [genWorthVals, setGenWorthVals] = useState<GenWorthValType>(
    () => INITIAL_GEN_WORTH_VALS
  );
  ...
  const fillGenWorthVals = () => {
    ...
    keys.forEach((key) => {
      const found = calculatedValues.find(
        (calcedVal) => calcedVal.traitCode.toLowerCase() === key
      );
      if (found) {
   ...
   */
  const setGenWorthInputObj = (
    genWorthName: keyof GenWorthValType,
    inputObj: StringInputType
  ) => {
    setGenWorthVals((prevVals) => ({
      ...prevVals,
   ...
   */
  const setGeoInfoInputObj = (
    infoName: keyof GeoInfoValType,
    inputObj: StringInputType
  ) => {
  ...
  const fillGwInfoItems = () => {
    ...
    keys.forEach((key) => {
      ...
      const traitIndex = calculatedValues
        .map((trait) => trait.traitCode)
        .indexOf(upperCaseCode);
      const tuple = clonedInfoItems[key];
      const traitValueInfoObj = tuple.filter((obj) =>
        obj.name.startsWith("Genetic")
      )[0];
      const testedPercInfoObj = tuple.filter((obj) =>
        obj.name.startsWith("Tested")
      )[0];
      if (calculatedValues[traitIndex]) {
        traitValueInfoObj.value = Number(
          calculatedValues[traitIndex].calculatedValue
        ).toFixed(1);

...
public interface TestResultRepository extends JpaRepository<TestResultEntity, BigDecimal> {
    ...
    @Modifying
    @Transactional
    @Query(value = "UPDATE CONSEP.CNS_T_TSC_TEST_RESULT SET TEST_COMPLETE_IND = 1 WHERE RIA_SKEY = (:riaKey)", nativeQuery = true)
    void updateTestResultStatusToCompleted(@Param("riaKey") BigDecimal riaKey);

    //Query to update the test result entity as accepted
    @Modifying
    @Transactional
    @Query(value = "UPDATE CONSEP.CNS_T_TSC_TEST_RESULT SET ACCEPT_RESULT_IND = 1 WHERE RIA_SKEY = (:riaKey)", nativeQuery = true)

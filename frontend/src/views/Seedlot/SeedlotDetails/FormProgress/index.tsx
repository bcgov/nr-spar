...
import { Button, ProgressIndicatorSkeleton, Row, Column } from "@carbon/react";
...
const FormProgress = ({
  ...
  useEffect(() => {
    if (
      seedlotStatusCode &&
      seedlotStatusCode !== "PND" &&
      seedlotStatusCode !== "INC"
    ) {
  ...
  useEffect(() => {
    if (
      formProgressStatusQuery.status === "success" &&
      (seedlotStatusCode === "PND" || seedlotStatusCode === "INC")
    ) {
    ...
    formProgressStatusQuery.isFetchedAfterMount,
  ]);
  ...
  const renderProgress = () => {
    if (
      ...
      if (
        getSeedlotQueryStatus === "loading" ||
        formProgressStatusQuery.status === "loading"
      ) {
      ...
      if (
        formProgressStatusQuery.status === "error" &&
        (formProgressStatusQuery.error as AxiosError).response?.status !== 404
      ) {
        return (
          <NetworkError
            description={(formProgressStatusQuery.error as AxiosError).message}
          />
        );
      }
    }
  ...
  return (
    <DetailSection title="See where you are in the registration process">
      <Row>
        <Column className="steps-box">{renderProgress()}</Column>
      </Row>
      <Row>
        <Column>
          <Button

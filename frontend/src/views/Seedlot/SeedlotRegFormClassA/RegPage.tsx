      ...
      <FlexGrid fullWidth>
          ...
          <Row>
            <Column>
              <ActionableNotification
                className={
                  saveStatus === "conflict"
                    ? "save-conflict-actionable-notification"
                    : "save-error-actionable-notification"
                }
                lowContrast
                kind="error"
                title={
                  saveStatus === "conflict"
                    ? `${smartSaveText.conflictTitle}`
                    : `${smartSaveText.error}:\u00A0`
                }
                subtitle={
                  saveStatus === "conflict"
                    ? smartSaveText.conflictSuggestion
                    : smartSaveText.suggestion
                }
                actionButtonLabel={
                  saveStatus === "conflict"
                    ? smartSaveText.reload
                    : smartSaveText.idle
                }
                onActionButtonClick={
                  saveStatus === "conflict" ? reloadFormDraft : handleSaveBtn
                }
              />
            </Column>
          </Row>
        ...
        <Row>
          <Column className="seedlot-registration-row">
            {isFetchingData ||
            submitSeedlot.status === "loading" ||
            (isFormSubmitted && !seedlotDataLoaded) ? (
              <Loading />
            ) : (
              <RegForm cleanParentTables={cleanParentTables} />
            )}
          </Column>
        </Row>

...
const MoistureContent = () => {
  ...
  const updateActivityRecordMutation = useMutation({
    mutationFn: (record?: ActivityRecordType) =>
      updateActivityRecord(activityRiaKey, record),
    onSuccess: () => {
      setAlert({
        isSuccess: true,
  ...
  useEffect(() => {
    if (
      ...
      setActivitySummary({
        ...
        speciesAndClass:
          `${seedlotQuery.data.seedlot.vegetationCode} | ${seedlotQuery.data.seedlot.geneticClass.geneticClassCode}` ||
          "",
  ...
  const averageTest = useMutation({
    mutationFn: (mcValues: number[]) =>
      calculateAverage(riaKey ?? "", mcValues),
    onSuccess: (data) => {
      const testActivityData: TestingActivityType = {
        ...testActivity!,
  ...
  const createBreadcrumbItems = () => {
    ...
    crumbsList.push({
      ...
      path: ROUTES.TESTING_REQUESTS_REPORT,
    });
    crumbsList.push({
      name: "Testing list",
  ...
  const buttons = [
    {
      ...
      action: () => {
        if (tableBodyRef.current) {
          ...
          const cells =
            tableBodyRef.current.querySelectorAll('td[data-index="6"]');
          const numbers = Array.from(cells).map((cell) =>
      ...
      <Row className="consep-moisture-content-title">
        <PageTitle
          title={`${fieldsConfig.titleSection.title} ${
            activitySummary && activitySummary.seedlotNumber
          }`}
        />
        <>
          ...
          <ComboBox
            ...
            value={
              ...
              activityRecord.testCategoryCode in categoryMap
                ? categoryMap[
                    activityRecord.testCategoryCode as keyof typeof categoryMap
                  ]
                : "Quality assurance"
            }
